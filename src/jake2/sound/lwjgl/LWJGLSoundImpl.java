...
//import org.lwjgl.openal.eax.EAX; // Huub 3 regels uit
...
//  private int currentEnv = EAX20.EAX_ENVIRONMENT_UNDERWATER; // Huub uit
...
//    int x = alGenSources(); // Huub

// Huub volgende allemaal uitgeschakeld.
//    AL.create(); 
//		try { 
//      AL.create(); 
...
// Huub volgende allemaal uitgeschakeld. try catch
//      try {
//        EAX.create();
...
//      }
  ...
  void exitOpenAL() { // Huub alles uit
...
//    EAX20.eaxSet(EAX20.LISTENER_GUID, EAXListenerProperties.EAXLISTENER_ENVIRONMENT | EAXListenerProperties.EAXLISTENER_DEFERRED, 0, eaxEnv, 4);
  ...
  sfx_t FindName(String name, boolean create) {
    ...
    // find a free sfx
    for (i = 0; i < num_sfx; i++) {
      if (known_sfx[i].name == null) // registration_sequence < s_registration_sequence)
      {
        break;
      }
    }
